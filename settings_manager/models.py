...
class SystemSetting(models.Model):
    ...
    display_name = models.CharField(
        max_length=200, help_text="Human-readable name shown in UI"
    )
    ...
    is_sensitive = models.BooleanField(
        default=False, help_text="Hide value in UI (for passwords, API keys)"
    )
    is_active = models.BooleanField(
        default=True, help_text="Whether this setting is currently in use"
    )
    editable_by_admin = models.BooleanField(
        default=True, help_text="Can be changed through UI or requires code change"
    )
    requires_restart = models.BooleanField(
        default=False, help_text="Requires server restart to take effect"
    )
...
class ActivityLog(models.Model):
    ...
    object_id = models.CharField(
        max_length=255, blank=True, help_text="ID of the affected object"
    )
    object_repr = models.CharField(
        max_length=255, blank=True, help_text="String representation of the object"
    )
    description = models.TextField(blank=True, help_text="Details about what happened")
    ip_address = models.GenericIPAddressField(null=True, blank=True)
    device_name = models.CharField(
        max_length=255, blank=True, help_text="Device hostname/name"
    )
    location = models.CharField(
        max_length=255, blank=True, help_text="Geographic location (City, Country)"
    )
    ...
    changes = models.JSONField(
        null=True, blank=True, help_text="Before/after values for updates"
    )
