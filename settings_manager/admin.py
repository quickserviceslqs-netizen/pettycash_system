...
class SystemSettingAdmin(admin.ModelAdmin):
    list_display = ('display_name', 'key', 'category', 'setting_type', 'is_active', 'last_modified_at')
    ...
    fieldsets = (
        ...
        ('Permissions & Behavior', {
            'fields': ('is_active', 'editable_by_admin', 'is_sensitive', 'requires_restart')
        }),
        ('Audit', {
...
class ActivityLogAdmin(admin.ModelAdmin):
    list_display = ('timestamp', 'user', 'action', 'content_type', 'object_repr', 'success')
    list_filter = ('action', 'success', 'timestamp')
    search_fields = ('user__username', 'description', 'object_repr')
    readonly_fields = ('user', 'action', 'timestamp', 'ip_address', 'user_agent', 
                      'content_type', 'object_id', 'description', 'changes', 'success', 'error_message')
