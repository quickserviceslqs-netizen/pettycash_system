    ...
    for key, expected_type, default_value in test_cases:
        try:
            ...
            if is_correct_type:
                ...
                print(f"❌ {key}: {value} (Expected {expected_type.__name__}, got {type(value).__name__})")
                all_passed = False
...
def summary_report():
    ...
    print(f"📁 Categories Covered: {len(by_category)}/{len(SystemSetting.CATEGORY_CHOICES)}")
    print()
