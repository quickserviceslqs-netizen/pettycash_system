...
class IntegrationTestBase(TransactionTestCase):
    ...
    def _create_user(self, username, email, role, branch=None, department=None,
                     region=None, company=None, is_staff=False, 
                     is_superuser=False, is_centralized=False):
    ...
    def create_requisition(self, requester, amount, purpose="Test requisition",
                          origin_type="branch", is_urgent=False):
    ...
    def approve_requisition(self, requisition, approver, notes="Approved"):
        ...
        if requisition.workflow_sequence:
            current_idx = next(
                (i for i, item in enumerate(requisition.workflow_sequence) 
                 if item["user_id"] == approver.id),
                None
            )
            
            if current_idx is not None and current_idx + 1 < len(requisition.workflow_sequence):
