...
def test_invitation_integration():
    ...
    uses_expiry = (
        "INVITATION_EXPIRY_DAYS" in source or "invitation_expiry" in source.lower()
    )
...
def test_notification_integration():
    ...
    notif_settings = SystemSetting.objects.filter(
        category="notifications", is_active=True
    )
...
def test_device_whitelist_integration():
    ...
    has_ip_middleware = any(
        "ip" in m.lower() or "whitelist" in m.lower() for m in middleware
    )
