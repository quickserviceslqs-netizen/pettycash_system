...
def download_template(request):
    ...
    if department_id:
        dept = (
            ...
            .select_related("branch__region__company")
            .first()
        )
        if dept:
        ...
        branch = (
            Branch.objects.filter(id=branch_id)
            .select_related("region__company")
            .first()
        )
        if branch:
    ...
    if "department" in filter_context:
        ...
        departments = Department.objects.filter(
            branch__region__company=filter_context["company"]
        )
    else:
    ...
    all_companies = (
        companies if filter_context else Company.objects.all().order_by("name")
    )
    ...
    if filter_context:
        all_regions = (
            ...
            or "branch" in filter_context
            or "department" in filter_context
            else Region.objects.filter(company__in=companies)
        )
    else:
        all_regions = Region.objects.all().order_by("name")[:20]
    for region in all_regions:
        company_info = (
            f" ({region.company.name})"
            if hasattr(region, "company") and region.company
            else ""
        )
    ...
    all_branches = (
        branches if filter_context else Branch.objects.all().order_by("name")[:20]
    )
    for branch in all_branches:
        region_info = (
            f" ({branch.region.name})"
            if hasattr(branch, "region") and branch.region
            else ""
        )
    ...
    all_departments = (
        departments
        if filter_context
        else Department.objects.all().order_by("name")[:20]
    )
    for dept in all_departments:
        branch_info = (
            f" ({dept.branch.name})" if hasattr(dept, "branch") and dept.branch else ""
        )
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="{"_".join(filename_parts)}_template.xlsx"'
    )
    wb.save(response)
            ...
            with transaction.atomic():
                for row_num, row in enumerate(
                    ...
                    try:
                        ...
                        if not row.get("email") or str(row.get("email", "")).startswith(
                            "INSTRUCTIONS"
                        ):
                            continue
                        ...
                        if role not in valid_roles:
                            errors.append(
                                f"Row {row_num}: Invalid role '{role}'. Must be one of: {', '.join(valid_roles)}"
                            )
                            error_count += 1
                            continue
                        ...
                        if UserInvitation.objects.filter(
                            ...
                            errors.append(
                                f"Row {row_num}: Pending invitation already exists for {email}"
                            )
                            error_count += 1
                            continue
                        ...
                        if row.get("company_name"):
                            ...
                            try:
                                ...
                                if companies.exists():
                                    ...
                                    errors.append(
                                        f"Row {row_num}: Company '{company_name}' not found. Check exact spelling."
                                    )
                                    error_count += 1
                                    continue
                        ...
                        if row.get("department_name"):
                            ...
                            try:
                                ...
                                if departments.exists():
                                    ...
                                    errors.append(
                                        f"Row {row_num}: Department '{dept_name}' not found. Check exact spelling."
                                    )
                                    error_count += 1
                                    continue
                            ...
                            try:
                                ...
                                if region_name:
                                    branch = Branch.objects.get(
                                        name=branch_name, region__name=region_name
                                    )
                                else:
                                ...
                                if branches.exists():
                                    ...
                                    errors.append(
                                        f"Row {row_num}: Branch '{branch_name}'{region_hint} not found. Check exact spelling."
                                    )
                                    error_count += 1
                                    continue
                            except Branch.MultipleObjectsReturned:
                                errors.append(
                                    f"Row {row_num}: Multiple branches named '{branch_name}' found. Please specify region_name."
                                )
                                error_count += 1
                                continue
                        ...
                        if row.get("assigned_apps"):
                            app_names = [
                                app.strip() for app in row["assigned_apps"].split(",")
                            ]
...
{request.user.get_full_name() or request.user.username}
                            ...
                            errors.append(
                                f"Row {row_num}: Email sent failed for {email}: {str(e)}"
                            )
            ...
            if success_count > 0:
                messages.success(
                    request, f"✅ Successfully created {success_count} invitation(s)"
                )
                log_activity(
                    request.user,
                    "USER_BULK_IMPORT",
            ...
            if error_count > 0:
                messages.warning(
                    request, f"⚠️ {error_count} row(s) failed. See details below."
                )
    ...
    context = {
        ...
        "departments": Department.objects.select_related("branch__region__company")
        .all()
        .order_by("branch__region__company__name", "name"),
