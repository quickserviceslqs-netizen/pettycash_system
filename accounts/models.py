...
class App(models.Model):
    ...
    display_name = models.CharField(
        max_length=100, help_text="User-friendly display name"
    )
    url = models.CharField(
        max_length=200, help_text="URL path for this app (e.g., /transactions/)"
    )
    description = models.TextField(blank=True, help_text="What this app does")
    is_active = models.BooleanField(
        default=True, help_text="Whether this app is currently available"
    )
...
class User(AbstractUser):
    ...
    ROLE_CHOICES = [
        (
            "admin",
            "Admin",
        ),  # Business admin - handles escalations, requires company assignment
        ("staff", "Staff"),
        ("treasury", "Treasury"),
        ("fp&a", "FP&A"),
...
class UserAuditLog(models.Model):
    ...
    timestamp = models.DateTimeField(
        default=timezone.now, help_text="When the action occurred"
    )
    changes = models.JSONField(
        null=True, blank=True, help_text="Details of what changed (before/after values)"
    )
    notes = models.TextField(
        blank=True, help_text="Additional context or reason for the change"
    )
    ip_address = models.GenericIPAddressField(
        null=True, blank=True, help_text="IP address of the admin who made the change"
    )
