...
def send_invitation(request):
    ...
    if request.method == "POST":
        ...
        if existing_pending and existing_pending.is_valid():
            messages.warning(
                request, f"A pending invitation already exists for {email}"
            )
            return redirect("accounts:invite_user")
        ...
        if invitation.send_invitation_email():
            ...
            messages.error(
                request,
                "Invitation created but email failed to send. Please try again.",
            )
...
def signup(request, token):
    ...
    if not invitation.is_valid():
        context = {"error": "expired", "invitation": invitation}
    ...
    if request.method == "POST":
        ...
        if len(password) < min_length:
            messages.error(
                request, f"Password must be at least {min_length} characters long"
            )
            return render(request, "accounts/signup.html", {"invitation": invitation})
        ...
        if require_complexity == "true":
            if not (
                ...
                messages.error(
                    request, "Password must contain uppercase, lowercase, and numbers"
                )
                return render(
                    request, "accounts/signup.html", {"invitation": invitation}
                )
...
def deactivate_device(request, device_id):
    ...
    if device.is_primary:
        messages.error(
            request,
            "Cannot deactivate primary device! Set another device as primary first.",
        )
    else:
        device.deactivate()
...
def admin_delete_device(request, device_id):
    ...
    if device_count == 1:
        messages.error(
            request,
            "Cannot delete the only device. User must have at least one device.",
        )
    ...
    if device.is_primary:
        messages.error(
            request,
            "Cannot delete primary device. Set another device as primary first.",
        )
        return redirect("accounts:manage_user_devices", user_id=user_id)
...
def admin_set_primary_device(request, device_id):
    ...
    if not device.is_active:
        messages.error(
            request, "Cannot set inactive device as primary. Activate it first."
        )
        return redirect("accounts:manage_user_devices", user_id=device.user.id)
...
def device_blocked(request):
    ...
    """
    context = {
        "support_email": get_setting(
            "SECURITY_ALERT_RECIPIENTS", "admin@company.com"
        ).split(",")[0],
    }
...
def request_device_registration(request):
    ...
    if request.method == "POST":
        ...
        existing_device = WhitelistedDevice.objects.filter(
            user=user, user_agent=device_info["user_agent"]
        ).first()
        ...
        if existing_device:
            if existing_device.is_active:
                messages.info(
                    request,
                    "This device is already whitelisted. Please try logging in again.",
                )
                return redirect("accounts:login")
            else:
                messages.info(
                    request,
                    "This device is registered but inactive. Please contact your administrator to activate it.",
                )
                return render(request, "accounts/request_device.html")
