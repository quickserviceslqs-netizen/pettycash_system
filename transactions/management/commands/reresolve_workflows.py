...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        pending_reqs = Requisition.objects.filter(status="pending").order_by(
            "created_at"
        )
        ...
        for req in pending_reqs:
            old_next_approver = (
                req.next_approver.username if req.next_approver else "None"
            )
            old_workflow_len = (
                len(req.workflow_sequence) if req.workflow_sequence else 0
            )

            self.stdout.write(f"\n{req.transaction_id}:")
            self.stdout.write(
                f"  Amount: {req.amount}, Origin: {req.origin_type}, Branch: {req.branch}"
            )
            self.stdout.write(
                f"  OLD: next_approver={old_next_approver}, workflow_steps={old_workflow_len}"
            )
            ...
            if req.workflow_sequence:
                for i, step in enumerate(req.workflow_sequence, 1):
                    ...
                    self.stdout.write(
                        f"    Step {i}: role={role}, user_id={user_id}{auto}"
                    )
