...
def manage_requisitions(request):
    ...
    requisitions = Requisition.objects.all().select_related(
        'requested_by', 'company', 'region', 'branch', 'department',
        'applied_threshold', 'next_approver'
    ...
    if my_requisitions:
        ...
        requisitions = requisitions.filter(next_approver=request.user, status__in=['pending', 'pending_urgency_confirmation'])
    ...
    stats = {
        ...
        'pending': Requisition.objects.filter(status__in=['pending', 'pending_urgency_confirmation']).count(),
        ...
        try:
            ...
            try:
                ...
                if threshold:
                    ...
                    messages.success(request, f'Requisition {requisition.transaction_id} created successfully! Applied threshold: {threshold.name}')
                else:
                    messages.warning(request, f'Requisition {requisition.transaction_id} created but no matching approval threshold found. Please contact admin.')
            except Exception as e:
                messages.warning(request, f'Requisition created but workflow setup failed: {str(e)}')
            
            return redirect('transactions:view_requisition', transaction_id=requisition.transaction_id)
...
def view_requisition(request, transaction_id):
    ...
    can_view = (
        requisition.requested_by == request.user or
        requisition.next_approver == request.user or
        request.user.has_perm('transactions.view_requisition')
    )
...
def approve_requisition(request, transaction_id):
    ...
    if request.method == 'POST':
        try:
            ...
            messages.success(request, f'Requisition {transaction_id} approved successfully!')
            return redirect('transactions:view_requisition', transaction_id=transaction_id)
...
def reject_requisition(request, transaction_id):
    ...
    can_reject = (
        requisition.next_approver == request.user or
        request.user.has_perm('transactions.delete_requisition')
    )
    ...
    if request.method == 'POST':
        try:
            ...
            if not reason:
                ...
                return redirect('transactions:reject_requisition', transaction_id=transaction_id)
