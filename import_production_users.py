...
for user_data in data['users']:
    try:
        ...
        for perm_data in user_data['permissions']:
            try:
                ct = ContentType.objects.get(app_label=perm_data['app_label'], model__isnull=False)
                # Find the permission - try multiple models
                perm = None
                for content_type in ContentType.objects.filter(app_label=perm_data['app_label']):
                    try:
                ...
                if perm:
                    ...
                    print(f"  ⚠️  Permission {perm_data['app_label']}.{perm_data['codename']} not found")
