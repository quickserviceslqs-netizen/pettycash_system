...
class SmokeTestRunner:
    ...
    def run_all(self):
        ...
        self.test("Requisitions API (unauthenticated)", "/api/requisitions/", expected_status=401)
        self.test("Treasury API (unauthenticated)", "/treasury/api/dashboard/metrics/", expected_status=401)
        
        # Dashboard
        self.test("Dashboard Redirect", "/dashboard/", expected_status=302)  # Redirects to login
        
        # Reports
        self.test("Reports Page", "/reports/", expected_status=302)  # Redirects to login
        
        # Database connection (via API)
        self.test("Database Connection", "/api/health/db/", 200)
        
        # Media files
        self.test("Media Directory", "/media/", expected_status=404)  # No files expected
