...
def test_settings_types():
    ...
    # Test boolean
    bool_settings = SystemSetting.objects.filter(setting_type='boolean', is_active=True).first()
    if bool_settings:
        val = bool_settings.get_typed_value()
        test_cases.append(('Boolean', bool_settings.key, type(val).__name__, isinstance(val, bool)))
    
    # Test integer
    int_settings = SystemSetting.objects.filter(setting_type='integer', is_active=True).first()
    if int_settings:
        val = int_settings.get_typed_value()
        test_cases.append(('Integer', int_settings.key, type(val).__name__, isinstance(val, int)))
    
    # Test JSON
    json_settings = SystemSetting.objects.filter(setting_type='json', is_active=True).first()
    if json_settings:
        val = json_settings.get_typed_value()
        test_cases.append(('JSON', json_settings.key, type(val).__name__, isinstance(val, (dict, list))))
...
def test_get_setting_function():
    ...
    non_existent = get_setting('NON_EXISTENT_SETTING_KEY_12345', default='fallback_value')
...
def test_category_coverage():
    ...
    print(f"  Coverage: {len(has_settings)}/{len(SystemSetting.CATEGORY_CHOICES)} categories ({coverage_pct:.1f}%)")
...
def test_editable_settings():
    ...
    editable = SystemSetting.objects.filter(is_active=True, editable_by_admin=True).count()
    locked = SystemSetting.objects.filter(is_active=True, editable_by_admin=False).count()
    requires_restart = SystemSetting.objects.filter(is_active=True, requires_restart=True).count()
    ...
    if requires_restart > 0:
        ...
        for setting in SystemSetting.objects.filter(is_active=True, requires_restart=True):
...
def test_settings_usage_in_code():
    ...
    all_keys = list(SystemSetting.objects.filter(is_active=True).values_list('key', flat=True))
