        ...
        for html_file in full_path.glob("**/*.html"):
            ...
            if dollar_matches or kes_matches:
                issues.append(
                    {
                        "file": str(
                            html_file.relative_to(r"C:\Users\ADMIN\pettycash_system")
                        ),
                        "type": "currency",
...
def find_hardcoded_timeouts():
    ...
    for file_path in files_to_check:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if "timedelta(" in line and any(
                x in line for x in ["minutes=", "hours=", "days="]
            ):
...
def print_report():
    ...
    if currency_issues:
        ...
        for issue in currency_issues:
            print(f"\n📄 {issue['file']}")
            print(
                f"   $ symbols: {issue['dollar_count']}, KES symbols: {issue['kes_count']}"
            )
            if issue["examples"]:
    ...
    if currency_issues:
        ...
        print(
            "  • Create currency_filters templatetag for workflow/treasury/system_maintenance"
        )
    ...
    if timeout_issues:
        ...
        print(
            "  • Use get_setting('SETTING_NAME', default) instead of hardcoded values"
        )
        print()
