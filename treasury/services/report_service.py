...
from treasury.models import (
    Payment, LedgerEntry, VarianceAdjustment, ReplenishmentRequest,
    TreasuryFund, FundForecast, DashboardMetric
)
...
class ReportService:
    ...
    @staticmethod
    def generate_payment_summary(company_id, start_date, end_date, region_id=None, branch_id=None):
        """
        ...
        for status in ['success', 'failed', 'pending']:
            ...
            by_status[status] = {
                ...
                'amount': status_query.aggregate(Sum('amount'))['amount__sum'] or Decimal('0.00'),
            }
        ...
        for method_dict in methods:
            ...
            by_method[method] = {
                ...
                'amount': method_query.aggregate(Sum('amount'))['amount__sum'] or Decimal('0.00'),
            }
        ...
        for origin_dict in origins:
            ...
            by_origin[origin] = {
                ...
                'amount': origin_query.aggregate(Sum('amount'))['amount__sum'] or Decimal('0.00'),
            }
        ...
        return {
            ...
            'by_method': by_method,
            'by_origin': by_origin,
            'success_rate': (by_status['success']['count'] / total_payments * 100) if total_payments > 0 else 0,
        }
    ...
    def generate_fund_health_report(company_id, region_id=None, branch_id=None):
        ...
        fund_details = []
        for fund in funds:
            utilization = (fund.current_balance / fund.reorder_level * 100) if fund.reorder_level > 0 else 0
        ...
        return {
            ...
            'avg_utilization_pct': float((total_balance / total_capacity * 100) if total_capacity > 0 else 0),
    ...
    def generate_variance_analysis(company_id, start_date, end_date):
        ...
        total_variance_amount = query.aggregate(Sum('variance_amount'))['variance_amount__sum'] or Decimal('0.00')
        
        positive_variances = query.filter(variance_amount__gt=0)
        positive_count = positive_variances.count()
        positive_amount = positive_variances.aggregate(Sum('variance_amount'))['variance_amount__sum'] or Decimal('0.00')
        
        negative_variances = query.filter(variance_amount__lt=0)
        negative_count = negative_variances.count()
        negative_amount = negative_variances.aggregate(Sum('variance_amount'))['variance_amount__sum'] or Decimal('0.00')
        
        # Pending approvals
        pending = query.filter(status='pending')
        pending_count = pending.count()
        pending_amount = pending.aggregate(Sum('variance_amount'))['variance_amount__sum'] or Decimal('0.00')
    ...
    def generate_replenishment_forecast(fund, horizon_days=30):
        ...
        predicted_balance = fund.current_balance - (daily_average * Decimal(horizon_days))
        predicted_balance = max(predicted_balance, Decimal('0.00'))
        
        # Calculate utilization
        predicted_utilization = (predicted_balance / fund.reorder_level * 100) if fund.reorder_level > 0 else 0
        predicted_utilization = min(predicted_utilization, 100)
        
        # Calculate days until reorder needed
        if daily_average > 0:
            days_until_reorder = int((fund.current_balance - fund.reorder_level) / daily_average)
        ...
        if needs_replenishment:
            recommended_amount = (fund.reorder_level * Decimal('1.5')) - predicted_balance
    ...
    def export_report_to_csv(report_data, report_type='payment_summary'):
        ...
        if report_type == 'payment_summary':
            ...
            writer.writerow(['Fund Name', 'Region', 'Branch', 'Balance', 'Reorder Level', 'Utilization %', 'Status'])
            ...
            writer.writerow(['Total Variance Amount', report_data['total_variance_amount']])
            writer.writerow(['Avg Approval Time (hours)', f"{report_data['avg_approval_time_hours']:.2f}"])
            writer.writerow([])
            
            writer.writerow(['Variance Summary'])
            writer.writerow(['Category', 'Count', 'Amount'])
            writer.writerow(['Positive', report_data['positive_variances']['count'], report_data['positive_variances']['amount']])
            writer.writerow(['Negative', report_data['negative_variances']['count'], report_data['negative_variances']['amount']])
            writer.writerow(['Pending', report_data['pending_approval']['count'], report_data['pending_approval']['amount']])
