...
class PaymentViewSet(viewsets.ModelViewSet):
    ...
    def verify_otp(self, request, payment_id=None):
        ...
        if not otp or len(otp) != 6:
            return Response(
                {"error": "OTP must be 6 digits"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def execute(self, request, payment_id=None):
        ...
        if success:
            ...
            return Response(
                {"success": False, "error": message}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def reconcile(self, request, payment_id=None):
        ...
        success, message = ReconciliationService.reconcile_payment(
...
class VarianceAdjustmentViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def approve(self, request, variance_id=None):
        ...
        if (
            ...
            and not request.user.groups.filter(name__icontains="cfo").exists()
        ):
            return Response(
        ...
        success, message = ReconciliationService.approve_variance(
            variance, request.user
        )
...
class LedgerEntryViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def by_fund(self, request):
        ...
        entries = LedgerEntry.objects.filter(fund__fund_id=fund_id).order_by(
            "-created_at"
        )
...
class AlertSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        read_only_fields = ["alert_id", "created_at", "acknowledged_at", "resolved_at"]
...
class FundForecastSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            ...
            "predicted_daily_expense",
            "days_until_reorder",
            "needs_replenishment",
            "recommended_replenishment_amount",
            "confidence_level",
...
class DashboardViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def summary(self, request):
        ...
        if not company_id:
            return Response(
                {"error": "User company not found"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class ReportingViewSet(viewsets.ViewSet):
    ...
    def payment_summary(self, request):
        ...
        start_date = request.query_params.get(
            "start_date", str(timezone.now().date() - timezone.timedelta(days=30))
        )
    ...
    def export(self, request):
        ...
        if report_type == "payment_summary":
            ...
            report_data = ReportService.generate_payment_summary(
                company_id, start_date, end_date
            )
            ...
            report_data = ReportService.generate_variance_analysis(
                company_id, start_date, end_date
            )
        else:
            return Response(
        ...
        if export_format == "csv":
            ...
            response["Content-Disposition"] = (
                f'attachment; filename="report_{report_type}.csv"'
            )
            return response
            ...
            if pdf_data:
                ...
                response["Content-Disposition"] = (
                    f'attachment; filename="report_{report_type}.pdf"'
                )
                return response
            else:
                return Response(
    ...
    try:
        ...
        try:
            ...
            if payment.status == "failed":
                payment.last_error = result.get(
                    "result_desc", "M-Pesa transaction failed"
                )
                payment.retry_count += 1

            payment.save(
                update_fields=["mpesa_receipt", "status", "last_error", "retry_count"]
            )
            ...
            if execution:
                ...
                execution.gateway_status = (
                    "success" if result.get("result_code") == 0 else "failed"
                )
        ...
        return JsonResponse(
            {"ResultCode": 1, "ResultDesc": f"Error processing callback: {str(e)}"}
        )
...
def treasury_dashboard(request):
    ...
    if not request.user.has_perm("treasury.view_treasuryfund"):
        messages.error(
            request,
            "You don't have permission to view treasury dashboard. Required: treasury.view_treasuryfund",
        )
...
def payment_execute(request):
    ...
    if not request.user.has_perm("treasury.change_payment"):
        messages.error(
            request,
            "You don't have permission to execute payments. Required: treasury.change_payment",
        )
        return redirect("dashboard")
...
def funds_view(request):
    ...
    if not request.user.has_perm("treasury.view_treasuryfund"):
        messages.error(
            request,
            "You don't have permission to view treasury funds. Required: treasury.view_treasuryfund",
        )
        return redirect("dashboard")
...
def alerts_view(request):
    ...
    if not request.user.has_perm("treasury.view_alert"):
        messages.error(
            request,
            "You don't have permission to view alerts. Required: treasury.view_alert",
        )
        return redirect("dashboard")
...
def variances_view(request):
    ...
    if not request.user.has_perm("treasury.view_varianceadjustment"):
        messages.error(
            request,
            "You don't have permission to view variances. Required: treasury.view_varianceadjustment",
        )
        return redirect("dashboard")
