...
from .models import (
    Payment, PaymentExecution, TreasuryFund, LedgerEntry, VarianceAdjustment, 
    ReplenishmentRequest, TreasuryDashboard, DashboardMetric, Alert, PaymentTracking, 
    FundForecast
)
...
class TreasuryDashboardAdmin(admin.ModelAdmin):
    list_display = ('company', 'total_balance', 'active_alerts', 'critical_alerts', 'calculated_at')
    list_filter = ('company', 'calculated_at')
    search_fields = ('company__name', 'region__name', 'branch__name')
    readonly_fields = (
        'dashboard_id', 'total_funds', 'total_balance', 'total_utilization_pct', 
        ...
        'pending_variance_amount', 'last_updated', 'calculated_at'
    )
...
class AlertAdmin(admin.ModelAdmin):
    list_display = ('alert_id', 'severity', 'alert_type', 'title', 'is_unresolved', 'created_at')
...
class PaymentTrackingAdmin(admin.ModelAdmin):
    list_display = ('tracking_id', 'payment', 'current_status', 'created_at', 'total_execution_time')
    list_filter = ('current_status', 'created_at')
    search_fields = ('payment__payment_id', 'tracking_id')
    readonly_fields = (
        'tracking_id', 'created_at', 'otp_sent_at', 'otp_verified_at',
        'execution_started_at', 'execution_completed_at', 'reconciliation_started_at',
        'reconciliation_completed_at', 'total_execution_time', 'otp_verification_time'
    )


@admin.register(FundForecast)
class FundForecastAdmin(admin.ModelAdmin):
    list_display = ('fund', 'forecast_date', 'predicted_balance', 'needs_replenishment', 'confidence_level')
