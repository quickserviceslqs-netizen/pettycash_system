...
class BudgetAllocationAdmin(admin.ModelAdmin):
	list_display = (
		'company', 'branch', 'department', 'cost_center', 'year', 'month', 'amount'
	)
	list_filter = (
		'company', 'year', 'month', 'branch', 'department'
	)
	search_fields = (
		'company__name', 'branch__name', 'department__name', 'cost_center__name'
	)
	...
	def export_selected_to_csv(self, request, queryset):
		...
		writer.writerow(['company_id', 'branch_id', 'department_id', 'cost_center_id', 'year', 'month', 'amount'])
		for obj in queryset:
			writer.writerow([
				obj.company_id,
				obj.branch_id or '',
	...
	def get_urls(self):
		urls = super().get_urls()
		custom = [
			path('import-csv/', self.admin_site.admin_view(self.import_csv_view), name='reports_budgetallocation_import_csv'),
		]
		return custom + urls
	...
	def import_csv_view(self, request):
		if request.method == 'POST' and request.FILES.get('csv_file'):
			...
			try:
				...
				created, updated = 0, 0
				for row in reader:
					company = Company.objects.get(id=int(row['company_id'])) if row.get('company_id') else None
					if not company:
						messages.error(request, 'company_id is required per row')
						return HttpResponseRedirect(reverse('admin:reports_budgetallocation_changelist'))
					branch = Branch.objects.filter(id=int(row['branch_id'])).first() if row.get('branch_id') else None
					department = Department.objects.filter(id=int(row['department_id'])).first() if row.get('department_id') else None
					cost_center = CostCenter.objects.filter(id=int(row['cost_center_id'])).first() if row.get('cost_center_id') else None
					year = int(row['year'])
				...
				return HttpResponseRedirect(reverse('admin:reports_budgetallocation_changelist'))
			messages.success(request, f'Import completed. Created: {created}, Updated: {updated}')
			return HttpResponseRedirect(reverse('admin:reports_budgetallocation_changelist'))
