...
from system_maintenance.models import (
    BackupRecord, RestorePoint, MaintenanceMode, 
    SystemHealthCheck, FactoryResetLog
)
...
def backup_management(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'create_backup':
            ...
            try:
                ...
                messages.success(request, f'Backup created successfully: {backup.backup_id}')
    ...
    total_size = backups.filter(status='completed').aggregate(
...
def restore_management(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'create_restore_point':
            ...
            restore_point = get_object_or_404(RestorePoint, restore_point_id=restore_point_id)
...
def health_check(request):
    ...
    if request.method == 'POST':
        ...
        messages.success(request, f'Health check completed. Score: {health_check.health_score}%')
        return redirect('system_maintenance:health_check_detail', check_id=health_check.check_id)
        ...
        if action == 'activate':
            ...
            if create_backup:
                try:
                    ...
                    messages.warning(request, f'Backup failed: {str(e)}. Continuing with maintenance activation.')
            ...
            messages.warning(request, 'Maintenance mode activated. Non-admin users cannot access the system.')
