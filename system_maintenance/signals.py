...
class RealTimeBackupManager:
    ...
    def __init__(self):
        ...
        self._backup_cooldown_minutes = getattr(settings, 'REALTIME_BACKUP_COOLDOWN', 30)
...
def backup_on_requisition_approved(sender, instance, created, **kwargs):
    ...
    if not created and instance.status == 'approved':
        ...
        if instance.amount >= getattr(settings, 'REALTIME_BACKUP_AMOUNT_THRESHOLD', 10000):
    ...
    if created or instance.status == 'disbursed':
        ...
        if instance.amount >= getattr(settings, 'REALTIME_BACKUP_AMOUNT_THRESHOLD', 10000):
            realtime_backup_manager.create_realtime_backup(
                ...
                user=instance.disbursed_by if hasattr(instance, 'disbursed_by') else None
            )
...
def backup_on_payment_deleted(sender, instance, **kwargs):
    ...
    realtime_backup_manager.create_realtime_backup(
        reason=f'Payment deleted: {instance.payment_id} (${instance.amount})',
        user=None
    )
...
def backup_on_superuser_created(sender, instance, created, **kwargs):
    ...
    if instance.is_superuser:
        if created:
            realtime_backup_manager.create_realtime_backup(
                reason=f'Superuser created: {instance.username}',
                user=None
            )
...
def backup_on_user_deleted(sender, instance, **kwargs):
    ...
    if instance.is_superuser or instance.is_staff:
        realtime_backup_manager.create_realtime_backup(
            reason=f'Staff/superuser being deleted: {instance.username}',
            user=None
        )
...
def backup_on_company_deleted(sender, instance, **kwargs):
    ...
    realtime_backup_manager.create_realtime_backup(
        reason=f'Company deleted: {instance.name}',
        user=None
    )
...
def backup_on_department_deleted(sender, instance, **kwargs):
    ...
    if hasattr(instance, 'users') and instance.users.exists():
        realtime_backup_manager.create_realtime_backup(
            reason=f'Department with users deleted: {instance.name}',
            user=None
        )
...
def backup_on_threshold_changed(sender, instance, created, **kwargs):
    ...
    if not created:  # Only on updates, not creation
        realtime_backup_manager.create_realtime_backup(
            reason=f'Approval threshold modified for {instance.role}',
            user=None
        )
...
def backup_on_threshold_deleted(sender, instance, **kwargs):
    ...
    realtime_backup_manager.create_realtime_backup(
        reason=f'Approval threshold deleted for {instance.role}',
        user=None
    )
