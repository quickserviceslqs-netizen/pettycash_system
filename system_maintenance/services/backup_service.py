...
class BackupService:
    ...
    def create_backup(self, backup_type="full", user=None, description=""):
        ...
        backup_id = (
            f"backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{os.urandom(4).hex()}"
        )
    ...
    def verify_backup(self, backup_record):
        ...
        try:
            # Check files exist
            if (
                backup_record.database_file
                and not backup_record.database_file.storage.exists(
                    backup_record.database_file.name
                )
            ):
                return False, "Database backup file not found"

            if (
                backup_record.media_archive
                and not backup_record.media_archive.storage.exists(
                    backup_record.media_archive.name
                )
            ):
            ...
            if backup_record.database_file:
                ...
                if actual_size != backup_record.database_size_bytes:
                    return (
                        False,
                        f"Database file size mismatch: expected {backup_record.database_size_bytes}, got {actual_size}",
                    )
    ...
    def cleanup_old_backups(self, retention_days=30, keep_protected=True):
        ...
        old_backups = BackupRecord.objects.filter(
            created_at__lt=cutoff_date, status="completed"
        )
    ...
    def get_backup_statistics(self):
        ...
        total_size = (
            BackupRecord.objects.filter(status="completed").aggregate(
