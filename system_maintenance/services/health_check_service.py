...
class HealthCheckService:
    ...
    def perform_health_check(self, user=None):
        ...
        check_id = f"health_{timezone.now().strftime('%Y%m%d_%H%M%S')}_{os.urandom(3).hex()}"
        ...
        health_check = SystemHealthCheck.objects.create(
            ...
            orphaned_records_count=len(self.checks_performed.get('orphaned_records', {}).get('orphaned_items', [])),
            missing_files_count=self.checks_performed.get('missing_files', {}).get('count', 0),
    ...
    def _check_disk_space(self):
        ...
        try:
            ...
            if disk_usage.percent > 90:
                ...
                self.recommendations.append('Consider cleaning up old files or expanding storage')
    ...
    def _check_backup_status(self):
        ...
        try:
            ...
            self.checks_performed['backup_status'] = {
                ...
                'total_backups': BackupRecord.objects.filter(status='completed').count()
            }
        except Exception as e:
            self.warnings.append({
                'check': 'backup_status',
    ...
    def _check_data_integrity(self):
        ...
        try:
            ...
            orphaned_requisitions = Requisition.objects.filter(requested_by__isnull=True).count()
            ...
            users_without_company = User.objects.filter(company__isnull=True, is_superuser=False).count()
            if users_without_company > 0:
                issues.append(f'{users_without_company} non-superuser users without company')
    ...
    def _check_orphaned_records(self):
        ...
        try:
            ...
            if orphaned_items:
                self.recommendations.append('Run data cleanup to remove orphaned records')
    ...
    def _check_missing_files(self):
        ...
        try:
            ...
            if missing_count > 0:
                ...
                self.recommendations.append('Review and clean up records with missing files')
