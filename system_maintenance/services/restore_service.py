...
class RestoreService:
    ...
    def create_restore_point(self, backup, name, description='', user=None):
        ...
        restore_point_id = f"restore_{timezone.now().strftime('%Y%m%d_%H%M%S')}_{os.urandom(4).hex()}"
    ...
    def restore_from_backup(self, backup_record, restore_database=True, 
                          restore_media=True, user=None):
        """
    ...
    def validate_restore_point(self, restore_point):
        ...
        # Check files exist
        if backup.database_file and not backup.database_file.storage.exists(backup.database_file.name):
            return False, "Database backup file missing"
        
        if backup.media_archive and not backup.media_archive.storage.exists(backup.media_archive.name):
