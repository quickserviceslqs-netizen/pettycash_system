...
class BackupRecord(models.Model):
    ...
    # File information
    database_file = models.FileField(
        upload_to="backups/database/", null=True, blank=True
    )
    media_archive = models.FileField(upload_to="backups/media/", null=True, blank=True)
    settings_file = models.FileField(
        upload_to="backups/settings/", null=True, blank=True
    )

    # Backup details
    file_size_bytes = models.BigIntegerField(
        default=0, help_text="Total backup size in bytes"
    )
    ...
    records_count = models.JSONField(
        default=dict, help_text="Count of records per model at backup time"
    )
    ...
    is_protected = models.BooleanField(
        default=False, help_text="Protected backups won't be auto-deleted"
    )
    ...
    def mark_completed(self):
        ...
        if self.created_at:
            self.duration_seconds = int(
                (self.completion_time - self.created_at).total_seconds()
            )
        self.save()
...
class RestorePoint(models.Model):
    ...
    backup = models.ForeignKey(
        BackupRecord, on_delete=models.CASCADE, related_name="restore_points"
    )
    ...
    system_state = models.JSONField(
        default=dict, help_text="Snapshot of critical system settings and state"
    )
...
class MaintenanceMode(models.Model):
    ...
    estimated_duration_minutes = models.IntegerField(
        default=30, help_text="Estimated maintenance duration"
    )
    ...
    def activate(self, user, reason, duration_minutes=None, backup=None):
        ...
        if duration_minutes is None:
            duration_minutes = SystemSetting.get_setting(
                "MAINTENANCE_WINDOW_DURATION_MINUTES", 30
            )
        ...
        self.expected_completion = self.activated_at + timezone.timedelta(
...
class SystemHealthCheck(models.Model):
    ...
    checks_performed = models.JSONField(
        default=dict, help_text="Detailed results of individual checks"
    )
...
class FactoryResetLog(models.Model):
    ...
    recovery_backup_id = models.CharField(
        max_length=100, blank=True, help_text="Backup ID for emergency recovery"
    )
