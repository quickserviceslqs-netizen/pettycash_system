...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--health-check", action="store_true", help="Run health check before backup"
        )
        parser.add_argument(
            "--cleanup",
            type=int,
    ...
    def handle(self, *args, **options):
        ...
        if options["cleanup"]:
            self.stdout.write(
                f'Cleaning up backups older than {options["cleanup"]} days...'
            )
            try:
                deleted_count = backup_service.cleanup_old_backups(
                    retention_days=options["cleanup"], keep_protected=True
                )
                self.stdout.write(
