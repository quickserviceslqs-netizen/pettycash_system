    ...
    if request.method == "POST":
        try:
            ...
            if not name or not origin_type or not roles_sequence:
                messages.error(
                    request, "Name, origin type, and at least one role are required."
                )
            ...
            messages.success(
                request, f'Approval threshold "{name}" created successfully!'
            )
...
def edit_threshold(request, threshold_id):
    ...
    if request.method == "POST":
        try:
            ...
            messages.success(
                request, f'Threshold "{threshold.name}" updated successfully!'
            )
            return redirect("workflow:manage_thresholds")
