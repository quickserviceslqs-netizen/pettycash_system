...
def workflow_dashboard(request):
    ...
    if not request.user.has_perm("workflow.view_approvalthreshold"):
        messages.error(
            request,
            "You don't have permission to view approval thresholds. Required: workflow.view_approvalthreshold",
        )
        return redirect("dashboard")
    ...
    fasttrack_count = all_thresholds.filter(
        allow_urgent_fasttrack=True, is_active=True
    ).count()
...
def threshold_edit(request, threshold_id):
    ...
    if not request.user.has_perm("workflow.change_approvalthreshold"):
        messages.error(
            request,
            "You don't have permission to edit approval thresholds. Required: workflow.change_approvalthreshold",
        )
    ...
    if request.method == "POST":
        try:
            ...
            threshold.allow_urgent_fasttrack = (
                request.POST.get("allow_urgent_fasttrack") == "on"
            )
            ...
            if not threshold.roles_sequence:
                messages.error(
                    request, "Please add at least one approver to the sequence."
                )
                context = {
                    "threshold": threshold,
            ...
            threshold.save()
            messages.success(
                request, f"Approval threshold '{threshold.name}' updated successfully!"
            )
            return redirect("workflow:dashboard")
