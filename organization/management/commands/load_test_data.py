...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        company, _ = Company.objects.get_or_create(
            name="Test Company Ltd", defaults={"code": "TEST001"}
        )

        # Create test region
        region, _ = Region.objects.get_or_create(
            name="Test Region", company=company, defaults={"code": "TR001"}
        )

        # Create test branch
        branch, _ = Branch.objects.get_or_create(
            name="Test Branch", region=region, defaults={"code": "TB001"}
        )

        # Create test departments
        finance_dept, _ = Department.objects.get_or_create(
            name="Finance", branch=branch
        )

        operations_dept, _ = Department.objects.get_or_create(
            name="Operations", branch=branch
        )
